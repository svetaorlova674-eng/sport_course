...
foreach($rentals as &$r){
    ...
    if($r['status'] === 'active' && $end_ts <= $now){
        // 1. Обновляем БД: статус -> completed
        $stmt = $pdo->prepare("UPDATE rent_history SET status='completed', actual_end_time=end_time WHERE id=?");
        $stmt->execute([$r['id']]);
        
        // 2. Освобождаем инвентарь
        $stmt = $pdo->prepare("UPDATE inventory SET status='free' WHERE id=?");
        $stmt->execute([$r['inventory_id']]);
        
        // 3. Обновляем локальную переменную, чтобы сразу на странице статус поменялся
        $r['status'] = 'completed';
